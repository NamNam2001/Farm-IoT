...
struct ConfigDevice
{
  ...
  uint32_t interval_wait;
  uint16_t times;
};

ConfigDevice XaNhapWrite;
ConfigDevice XaNhapRead;
...
void EEPROM_writeStruct(int address, const ConfigDevice &deviceConfig)
{
...
void EEPROM_readStruct(int address, ConfigDevice &deviceConfig)
{
...
void onEvent(ev_t ev)
{
  ...
  case EV_TXCOMPLETE:
    ...
    if (LMIC.dataLen)
    {
      ...
      if (LMIC.dataLen == 1)
      {
        uint8_t times = receivedData[0];
        XaNhapWrite.times = times;
        EEPROM_writeStruct(AddressWrite, XaNhapWrite);
      }
      else if (LMIC.dataLen == 3)
      {
        // timestamp xanhap config
        uint32_t interval_wait = ((uint32_t)receivedData[0] << 16 |
                                  (uint32_t)receivedData[1] << 8 |
                                  (uint32_t)receivedData[2] << 0);
        XaNhapWrite.interval_wait = interval_wait;
        EEPROM_writeStruct(AddressWrite, XaNhapWrite);
      }
      else if (LMIC.dataLen == 4)
      {
      ...
      else if (LMIC.dataLen == 6)
      {
        uint32_t timeStart_xanhap_config = ((uint32_t)receivedData[0] << 16 |
                                            (uint32_t)receivedData[1] << 8 |
                                            (uint32_t)receivedData[2] << 0);

        uint32_t interval_run = ((uint32_t)receivedData[3] << 16 |
                                 (uint32_t)receivedData[4] << 8 |
                                 (uint32_t)receivedData[5]);
        // Serial.println(timeStart_xanhap_config);
        // Serial.println(interval_run);
        XaNhapWrite.timeStart = timeStart_xanhap_config;
        XaNhapWrite.interval_run = interval_run;

        EEPROM_writeStruct(AddressWrite, XaNhapWrite);
      }
      EEPROM_readStruct(AddressWrite, XaNhapRead);
    }
    // data = counter;
...
static void vHandleControllerTask(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    if (abs((currentTime % 86400) - XaNhapRead.timeStart - nextTime) <= 1)
    {
      Serial.println("Running...");
      digitalWrite(RELAY1, HIGH);
      digitalWrite(RELAY2, LOW);
    }
    if (abs((currentTime % 86400) - XaNhapRead.timeStart - XaNhapRead.interval_run - nextTime) <= 1)
    {
      Serial.println("Close...");
      digitalWrite(RELAY1, HIGH);
      digitalWrite(RELAY2, LOW);
      times = (times + 1 < XaNhapRead.times) ? times + 1 : 0;
      nextTime = (times < XaNhapRead.times) ? times * (XaNhapRead.interval_run + XaNhapRead.interval_wait) : 0;
    }

    Serial.print("Remaining: ");
    Serial.println(abs((rtc.getTime() % 86400) - XaNhapRead.timeStart));
    Serial.println("Remaining NextTime: ");
    Serial.println(abs((rtc.getTime() % 86400) - XaNhapRead.timeStart - XaNhapRead.interval_run));
    ...
    Serial.println(XaNhapRead.timeStart);

    Serial.print("interval_run: ");
    Serial.println(XaNhapRead.interval_run);

    Serial.print("interval_wait: ");
    Serial.println(XaNhapRead.interval_wait);

    Serial.print("times: ");
    Serial.println(XaNhapRead.times);
    vTaskDelay(1000);
  }
}
